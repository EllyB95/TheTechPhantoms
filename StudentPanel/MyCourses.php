
<!DOCTYPE html>
<html lang="en-CA" class="no-js">
<head>
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js"></script>
<link href='../css/studentStyle.css' rel='stylesheet' type="text/css"/>
<link href='../css/admin_table.css' rel='stylesheet' type="text/css"/>
<script src="https://code.iconify.design/1/1.0.7/iconify.min.js"></script>
<svg style="display:none;">
</svg>
</head>
<body>
  <header class="page-header">
    <nav>
      <a href="StudentDashboard.php">
        <img class="logo" src="../images/png/cropped-RecoveryCollege_Favicon-180x180.png" alt="Recovery College logo">
      </a>   
      <ul class="admin-menu">
        <!--<li class="menu-heading">
          <h2 style="color: #141313">Student Portal</h2>
        </li> -->
        <li>
          <a href="StudentDashboard.php">
              <span class="iconify" data-icon="fa-solid:house-user" data-inline="false"></span>
            <span>Dashboard</span>
          </a>
        </li>
        <li>
          <a href="MyCourses.php">
              <span class="iconify" data-icon="mdi:bookshelf" data-inline="false"></span>
            <span>My Courses</span>
          </a>
        </li>
        <li>
          <a href="MyAccount.php">
              <span class="iconify" data-icon="carbon:account" data-inline="false"></span>
            <span>My Account</span>
          </a>
        </li>
        <li>
          <a href="Certificates.php">
              <span class="iconify" data-icon="grommet-icons:certificate" data-inline="false"></span>
              <span>Certificates</span>
            </a>
        </li>
        <li>
          <a href="AcademicSupport.php">
              <span class="iconify" data-icon="teenyicons:users-solid" data-inline="false"></span>
              <span>Academic Support</span>
            </a>
        </li>
        <li>
      <a href="/LoginRegistrationPages/logout.php"  style="color: #141313">
              <h4>Sign Out</h4>
          </a>
      
            <!--button type="button" style="color: #141313"><h4>Sign Out</h4></button-->  
        </li>
        <br>
      </ul>
    </nav>
  </header>
  <section class="page-content">
    <section class ="grid">
      <article style="height: 550px">
        <div class="main__container">
          <div class="main__title">
              <div class="main__greeting">
                <h1>Student Information</h1>
              </div>
              </div>
            <table class="content-table">
              <thead>
                <tr>
                <th>Sr.No</th>
                <th>Course Name</th>
                </tr>
              </thead>
              <tbody>
              <?php 
               include_once("../php/config.php");
               include_once("../php/sql.php");
               $count = 1; ?>
              
              <?php while ($row = pg_fetch_array($contact)) {   ?>
                  <tr>
                    <td><?php  echo  $count; ?></td>
                    <td><a href="../Courses.php"><?php echo $row['coursename']; ?></a></td>
                 
                  </tr>
                <?php $count++ ?>             
              <?php }   ?>
              </tbody>
            </table>
          </div>
        </article>
    </section>
  </section>
</body>
<footer class="page-footer">
</footer>